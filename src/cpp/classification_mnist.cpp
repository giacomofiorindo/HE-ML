...
int main(int argc, char *argv[]) {
        ...
        if (argc < 2) mode = false;
        else mode = std::stoi(argv[1]);

        if (mode) {
                extract_array("../../resources/mnist/weights_and_biases/q_biases0.csv", biases[0]);
                extract_array("../../resources/mnist/weights_and_biases/q_biases1.csv", biases[1]);
                extract_array("../../resources/mnist/weights_and_biases/q_weights0.csv", weights[0]);
                extract_array("../../resources/mnist/weights_and_biases/q_weights1.csv", weights[1]);
        } else {
                extract_array("../../resources/mnist/weights_and_biases/q_30_biases0.csv", biases[0]);
                extract_array("../../resources/mnist/weights_and_biases/q_30_biases1.csv", biases[1]);
                extract_array("../../resources/mnist/weights_and_biases/q_30_weights0.csv", weights[0]);
                extract_array("../../resources/mnist/weights_and_biases/q_30_weights1.csv", weights[1]);
        }

        extract_array("../../resources/mnist/input_mnist_test.csv", input);
        extract_array("../../resources/mnist/labels_mnist_test.csv", labels);
        ...
        int message_space = 800;
        LweSample *multi_sum, *bootstrapped_multi_sum;
        int layers = 3;
        int neurons[3] = {784, 30, 10};
        if (mode) neurons[1] = 300;
        double correct, total = 0;

        LweSample *enc_input = new_LweSample_array(784, secret_key->params->in_out_params);
        LweSample *result = new_LweSample_array(10, secret_key->params->in_out_params);
        ...
        if (argc < 3) num_threads = omp_get_num_threads();
        else num_threads = std::stoi(argv[2]);
        if (argc < 4) limit = 10000;
        else limit = std::stoi(argv[3]);
        if (argc < 5) start = 0;
        else start = std::stoi(argv[4]);
        ...
        for (int k = start; k < start + limit; ++k) {
                ...
                for (int i = 0; i < 784; ++i) {
                        mu = modSwitchToTorus32(input[k * 784 + i], message_space);
        ...
        delete_LweSample_array(784, enc_input);
        delete_LweSample_array(10, result);
